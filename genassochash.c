...
int main(int argc, char **argv)
{
    ...
    if (argc != 3) {
        fprintf(stderr, "./genassochash <key> <hashpower>\n");
        exit(-1); 
    }
    ...
    hval = mc_hash(kbuf, klen, 0);
    hidx = hval & hashmask(hpwr);
    hsiz = hashsize(hpwr);
	fprintf(stderr, "key=%s length=%d hval=%u hidx=%d(size=%d, hpwr=%d)\n",
            kbuf, klen, hval, hidx, hsiz, hpwr);
    return 0;
}
