...
ZK_ADDR=\"-server ${ZK_ADDR_URL}:${ZK_PORT}\"
...
echo "ZK_CLI=\"${ZK_CLI_STR}\"
ZK_ADDR=\"-server ${ZK_ADDR_URL}:${ZK_PORT}\"" > ${REMOVE_FILE_NAME}
...
\$ZK_CLI \$ZK_ADDR delete /arcus_repl/cache_server_mapping/${NODE_ADDR}:${NODE_PORT}" >> ${REMOVE_FILE_NAME}
    ...
    eval START_MEMC_FILE_NAME_OUT=${START_MEMC_FILE_NAME}-${NODE_ADDR}.bash;
    echo "mkdir -p ${ARCUS_INST_DIR}/memc_pid_list

${ARCUS_INST_DIR}/bin/memcached -P ${ARCUS_INST_DIR}/memc_pid_list/memcached.${NODE_ADDR}:${NODE_PORT} -E ${ARCUS_INST_DIR}/lib/default_engine.so -X ${ARCUS_INST_DIR}/lib/syslog_logger.so -X ${ARCUS_INST_DIR}/lib/ascii_scrub.so -d -v -o 60 -r -R5 -U 0 -D: -b 8192 -m${NODE_MEM} -p ${NODE_PORT} -c ${MAX_CONNS} -t 6 -u ${MEMC_USER_ACCOUNT} -z ${ZK_ADDR_URL}:${ZK_PORT}" >> ${START_MEMC_FILE_NAME_OUT}
